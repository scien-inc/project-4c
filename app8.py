...
# 1) フォルダ内の .json ファイルをすべて取得
json_paths = glob.glob(os.path.join(JSON_FOLDER, "*.json"))
if not json_paths:
    st.warning(f"指定フォルダ({JSON_FOLDER})に json ファイルがありません。")
    st.stop()
    ...
    for proj_idx, project in enumerate(data["DXProjects"]):
        ...
        with st.expander(f"[{company_name}] / 課題: {purpose}", expanded=False):
            ...
            if st.button(f"『{company_name}』の評価を保存", key=save_button_key):
                ...
                # ここでは「一社分だけ」を JSON 化して保存するイメージ
                # 必要に応じて、同じファイルに複数社分を入れたい場合はご自由に変更ください。
                single_project_data = {
                    "DXProjects": [project]
                }
                ...
                # 保存先を同じフォルダ内とする
                out_path = os.path.join(JSON_FOLDER, out_filename)
                ...
                # ダウンロードボタン（ブラウザからもDLできるようにしたい場合）
